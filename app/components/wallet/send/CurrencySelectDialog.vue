<script setup>
import SendCurrencyDialog from "./SendCurrencyDialog";
...
const sendCurrencyDialogShow = ref(false);
...
const goToSendCurrency = () => {
  sendCurrencyDialogShow.value = true;
};
      ...
      <v-card-text class="fill-height d-flex flex-column">
        <div>
          ...
          >
            ...
            <div class="mr-auto ml-2">{{ $t("send") }}</div>
        ...
        <div>
          ...
          <div class="mt-auto" style="padding-bottom: 4px; width: 100%">
            <TMAMainButton
              color="primary"
              :title="$t('continue')"
              @onClick="goToSendCurrency"
              size="large"
            />
          </div>
        </div>
      </v-card-text>
      <SendCurrencyDialog v-model="sendCurrencyDialogShow" />
